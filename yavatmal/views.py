...
def Appappear_exam(request, T_Exam_ExamID_url):
    ...
    return render(request,
    'site/App/appear_exam.html', context_dict )
...
def Appexam_answers(request, T_Exam_ExamID_url):
    ...
    return render(request,
    'site/App/exam_answers.html', context_dict )
...
from .serializers import T_QuestionsSetSerializer

class QuestionList(APIView):

    def get(self, request):
        ques = T_QuestionsSet.objects.all()
        serializer = T_QuestionsSetSerializer(ques, many=True)
        return Response(serializer.data)

    def post(self, request):
        serializer = T_QuestionsSetSerializer(data=request.data)
    ...
    def questionsset_detail(request, pk):
        """
        Retrieve, update or delete a code snippet.
        """
        try:
            ques = T_QuestionsSet.objects.get(QueID=pk)
        except ques.DoesNotExist:
            return HttpResponse(status=404)

        if request.method == 'GET':
            serializer = T_QuestionsSetSerializer(ques)
            return JsonResponse(serializer.data)

        elif request.method == 'PUT':
            data = JSONParser().parse(request)
            serializer = T_QuestionsSetSerializer(ques, data=data)
            if serializer.is_valid():
                serializer.save()
                return JsonResponse(serializer.data)
